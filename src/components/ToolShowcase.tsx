    ...
    <section id="tools" className="py-16 bg-secondary/30">
      <div className="container">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Explore Our Premium Tools</h2>
          ...
          {categories.map((category) => (
            <div key={category}>
              ...
              <div className="flex justify-center">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-3 gap-4 justify-center max-w-4xl mx-auto">
                  {tools
                    .filter(tool => tool.category === category)
                    .map(tool => (
                      <Card key={tool.name} className="flex flex-col items-center p-4 card-hover">
                        <img src={tool.logo} alt={tool.name} className={`${tool.sizeClassName || 'w-12 h-12'} m-6`} />
